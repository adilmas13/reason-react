...
let make = () => {
  ...
  <div>
    ...
    <br />
    <div>
      {string("-------- Conditional State Change using same function -----")}
    </div>
    <br />
    <button onClick={__x => conditionalUpdate(__x, true)}>
      {string("Increment Counter 3")}
    </button>
    <button onClick={__x => conditionalUpdate(__x, false)}>
      {string("Decrement Counter 3")}
    </button>

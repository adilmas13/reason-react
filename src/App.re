...
let make = () => {
  ...
  // make function is part of ReactDOMRe.Style module which is opened on top
  let bodyStyle =
    make(~flex="1", ~padding="20px", ~boxSizing="border-box", ());

  let bodyWrapperStyle =
    make(~display="flex", ~flexDirection="column", ~flex="1", ());
  ...
  <ReasonReactErrorBoundary
    ...
    <div style=parentStyle>
      <NavigationPanel />
      <div style=bodyWrapperStyle> <Header /> {body()} <References /> </div>
    </div>
  </ReasonReactErrorBoundary>;
};